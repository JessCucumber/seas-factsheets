...
let container = document.getElementById("viz_container");
...
// Append SVG object
let svg = d3.select("#viz_container").append("svg")
...
// Determine the number of ticks based on screen width
let numTicks;
if (window.innerWidth <= 480) {
  numTicks = 5; // For mobile screens
} else if (window.innerWidth <= 768) {
  numTicks = 7; // For tablet screens
} else {
  numTicks = 10; // For desktop screens
}
...
svg.append("text")
  ...
  .text(yAxisLabelText);
    ...
    // Get the last data points for each line
    const lastEthanolData = ethanolData[ethanolData.length - 1];
    const lastBiodieselData = biodieselData[biodieselData.length - 1];

    // Define the labels
    const labels = [
      { label: 'Ethanol', color: '#32A28D', data: lastEthanolData, scale: y1 },
      { label: 'Biodiesel', color: '#A6B93D', data: lastBiodieselData, scale: y0 }
    ];

    // Append labels to the SVG
    labels.forEach((label) => {
      svg.append('text')
        .attr('x', x(label.data.year)) // Position labels at the x position of the last data point
        .attr('y', label.scale(label.data.value) - 10) // Subtract 10 (or any other value that suits your needs) from the y position
        .attr('text-anchor', 'start') // Left align labels
        .style('fill', label.color) // Use the same color as the lines
        .text(label.label);
    });
